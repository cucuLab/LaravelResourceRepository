...
//            }
                ...
                foreach ($data as $each) {
                        if($tableName == $each->resource_table){
                            ...
                            if(!array_key_exists('id', $filterArray))
                            {
                                $filterArray['id']= [
                                    'include' => [],
                                    'exclude' => []
                            ...
                            if($each->access == 1)
                            {
                                $filterArray['id']['include'][] = $each->resource_id;
                            }else{

                                $filterArray['id']['exclude'][] = $each->resource_id;
                            }
//                    if($each->access == 1)
//                    {
//                        $this->currentQuery->where('id', '=', $each->resource_id);
//                    }
                        }
                        else if(\Schema::hasColumn($tableName, $each->resource_type))
                        {
                            if(!array_key_exists($each->resource_type, $filterArray))
                            {
                                $filterArray[$each->resource_type]= [
                                    'include' => [],
                                    'exclude' => []
                                ];
                            }
                            if($each->access == 1)
                            {
                                $filterArray[$each->resource_type]['include'][] = $each->resource_id;
                            }else{

                                $filterArray[$each->resource_type]['exclude'][] = $each->resource_id;
                            }
//                    $this->currentQuery->where($each->resource_type, "=", $each->resource_id);
                        }

                }
//        dd($filterArray);
                foreach ($filterArray as $key => $each) {
                    if(!empty($each['include']))
                    {
                        $this->currentQuery->whereIn($key, $each['include']);
                    }
                    if(!empty($each['exclude']))
                    {
